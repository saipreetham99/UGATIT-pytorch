...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str,
                        required=True, help='path to the checkpoint file')
    ...
    parser.add_argument('--direction', type=str, default='A2B',
                        choices=['A2B', 'B2A'], help='translation direction')
    parser.add_argument('--img_size', type=int, default=256,
                        help='image size used during training')
    parser.add_argument('--light', type=str2bool, default=False,
                        help='was the model trained in light mode?')
    parser.add_argument('--ch', type=int, default=64,
                        help='base channel number')
...
def main():
    ...
    generator = ResnetGenerator(input_nc=3, output_nc=3, ngf=args.ch, n_blocks=args.n_res,
                                img_size=args.img_size, light=args.light).to(args.device)
    ...
    try:
        ...
        if image_bgr is None:
            raise FileNotFoundError(
                f"Could not read image file: {args.input_image}")
